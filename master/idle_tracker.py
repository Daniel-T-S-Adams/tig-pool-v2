...
Keep in sync with pool_manager/pool/idle_tracker.py (separate Docker images).
"""
...
def idle_window_settings() -> dict:
    return {
        ...
        # Fraction of the observed window that must be idle to count as sustained.
        "frac_threshold": min(
            1.0,
            max(0.0, float(os.environ.get("IDLE_WINDOW_FRAC_THRESHOLD", "0.5"))),
        ),
        # Until this much observation accumulates, require continuous idle instead
        # of trusting the fraction (avoids startup and flicker).
        "min_observed_ms": max(
            0,
            int(os.environ.get("IDLE_WINDOW_MIN_OBSERVED_MS", str(30_000))),
        ),
        # During warm-up, only count currently-idle slaves idle at least this long.
...
# Process singletons (master create path / optional local tests).
