...
def build_worker_earnings(
    *,
    ...
    for row in workers:
        ...
        # 12h / 24h / joined are slices of this slave's own week TIG.
        # If they only worked today, 24h and joined match Round.
        row["est_tig_1h"] = allocate_tig(row["nonces_1h"], row["nonces"], row["est_tig"])
        row["est_tig_12h"] = allocate_tig(row["nonces_12h"], row["nonces"], row["est_tig"])
        row["est_tig_24h"] = allocate_tig(row["nonces_24h"], row["nonces"], row["est_tig"])
    ...
    payload = {
        ...
        "note": (
            "Approximate. Round is this wallet's current-round payout split. "
            "24h / 12h are the part of that same TIG from work in that window. "
            "Joined matches Round when all of this slave's round work is after it showed up."
        ),
        "workers": workers,
    }
    _cache["data"] = payload
