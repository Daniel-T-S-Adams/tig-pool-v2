...
async function loadOperator() {
  ...
  table('operator-withdrawals',payments.withdrawals.map(row=>{const actions=node('div');if(row.state==='requested')actions.append(button('Approve',()=>reviewWithdrawal(row,true)),button('Reject',()=>reviewWithdrawal(row,false),'danger'));if(row.state==='approved')actions.append(button('Prepare payment',()=>prepareWithdrawal(row)),button('Reject',()=>reviewWithdrawal(row,false),'danger'));if(row.state==='uncertain'){const attempt=payments.attempts.find(value=>value.withdrawal_id===row.id&&!value.outcome);if(attempt)actions.append(button('Check transfer',async()=>showPayment(await api('operator/withdrawal-attempts/'+attempt.id))));}return[address(row.wallet),tig(row.amount),address(row.recipient),status(row.state),actions];}),5);
